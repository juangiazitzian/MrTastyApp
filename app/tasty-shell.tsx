...
import { LayoutDashboard, Package, ReceiptText, ChartNoAxesCombined, Users, Link2, Store, ArrowUpRight, ArrowDownRight, ChevronRight, Sun, Moon, X } from 'lucide-react';
...
import { StatusStrip } from './status-bar';
...
/**
 * Detalle del mes elegido en el gráfico.
 *
 * Además de contestar «por qué ese mes», salda una obligación de accesibilidad:
 * el naranja de marca no llega a 3:1 contra el fondo claro, y esa holgura se
 * compensa con una vista en tabla donde cada valor se lee sin depender del
 * color, del hover ni de distinguir una barra de la otra.
 *
 * Las alertas del período se muestran tal cual vienen. Son las diferencias que
 * el equipo decidió conservar en vez de emprolijar, y esconderlas acá sería
 * justo lo contrario de lo que se busca.
 */
function PeriodDetail({ period, finances, location, onClose, onOpenResults }: {
    period: string;
    finances: Finance[];
    location: string;
    onClose: () => void;
    ...
    const rows = finances.filter(f => f.period === period && (location === 'all' || f.location === location));
    if (!rows.length)
        return <p className="period-empty">No hay datos cargados para {periodLabel(period)}.</p>;
    const valor = (r: Finance, lista: 'groups' | 'channels', label: string) => r[lista].find(x => x.label === label)?.value ?? null;
    const etiquetas = (lista: 'groups' | 'channels') => Array.from(new Set(rows.flatMap(r => r[lista].map(x => x.label))));
    const alertas = rows.flatMap(r => r.flags.map((f, i) => ({ id: `${r.id}-${i}`, local: names[r.location], texto: f })));
    const desglose = (titulo: string, lista: 'groups' | 'channels') => {
        const labels = etiquetas(lista);
        if (!labels.length)
            return null;
        return <><h4>{titulo}</h4><Table><TableHeader><TableRow><TableHead>{lista === 'groups' ? 'Categoría' : 'Canal'}</TableHead>{rows.map(r => <TableHead key={r.id}>{names[r.location]}</TableHead>)}</TableRow></TableHeader><TableBody>{labels.map(label => <TableRow key={label}><TableCell>{label}</TableCell>{rows.map(r => <TableCell key={r.id}>{money(valor(r, lista, label))}</TableCell>)}</TableRow>)}</TableBody></Table></>;
    };
    return <div className="period-detail">
        <div className="period-detail-head">
            <div><h3>{periodLabel(period)}</h3><p>Detalle del mes · pesos argentinos</p></div>
            <button className="icon-button" onClick={onClose} aria-label="Cerrar el detalle del mes"><X size={16}/></button>
        </div>
        <Table><TableHeader><TableRow><TableHead>Concepto</TableHead>{rows.map(r => <TableHead key={r.id}>{names[r.location]}</TableHead>)}</TableRow></TableHeader><TableBody>
            <TableRow><TableCell>Facturación</TableCell>{rows.map(r => <TableCell key={r.id}>{money(r.revenue)}</TableCell>)}</TableRow>
            <TableRow><TableCell>Gastos</TableCell>{rows.map(r => <TableCell key={r.id}>{money(r.expenses)}</TableCell>)}</TableRow>
            <TableRow><TableCell>Resultado</TableCell>{rows.map(r => <TableCell key={r.id}>{r.complete ? money(r.profit) : 'Pendiente'}</TableCell>)}</TableRow>
            <TableRow><TableCell>Margen</TableCell>{rows.map(r => <TableCell key={r.id}>{r.complete ? pct(r.margin) : 'Pendiente'}</TableCell>)}</TableRow>
        </TableBody></Table>
        {desglose('Gasto por categoría', 'groups')}
        {desglose('Ventas por canal', 'channels')}
        {!!alertas.length && <ul className="period-flags">{alertas.map(a => <li key={a.id}><strong>{a.local}:</strong> {a.texto}</li>)}</ul>}
        <button className="text-button" onClick={onOpenResults}>Abrir en Estado de resultados <ArrowUpRight size={15}/></button>
    </div>;
}
...
export default function TastyShell() {
    ...
    // Sin registros del período no hay gasto cero: no hay dato. Sumar sobre un
    // array vacío daría "$ 0 M" y parecería un mes sin gastos.
    const expenses = records.length ? records.reduce((a, f) => a + f.expenses, 0) : null;
    ...
    const chart = Array.from(new Set(finances.map(f => f.period))).filter(p => p <= period).sort().slice(-6).map(p => ({ key: p, period: periodLabel(p), ...Object.fromEntries(['balbin', 'peron'].map(loc => [loc, finances.find(f => f.period === p && f.location === loc)?.revenue ?? null])) }));
    return <SidebarProvider><Sidebar className="tasty-sidebar"><SidebarHeader><div className="official-brand"><img src="/brand/logo.png" alt="Mr. Tasty" width="142" height="66"/><span>OPERACIONES</span></div></SidebarHeader><SidebarContent><div className="nav-label">SAN MIGUEL</div><SidebarMenu>{nav.map(n => <SidebarMenuItem key={n.id}><SidebarMenuButton isActive={view === n.id} onClick={() => setView(n.id)}><n.icon /><span>{n.label}</span>{view === n.id && <ChevronRight className="ml-auto"/>}</SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarContent><SidebarFooter><div className="store-note"><Store size={18}/><div>MR. TASTY SAN MIGUEL<small>Balbín & Perón</small></div></div><div className="owner"><span>MT</span><div>Gestión Mr Tasty<small>San Miguel, Buenos Aires</small></div></div></SidebarFooter></Sidebar><div className="workspace"><header className="topbar"><div className="breadcrumb"><SidebarTrigger /><span>Operaciones</span><ChevronRight size={14}/><strong>{nav.find(n => n.id === view)?.label}</strong></div><StatusStrip/><div className="top-actions"><button className="icon-button" onClick={toggleTheme} aria-label={dark ? "Cambiar a tema claro" : "Cambiar a tema oscuro"} title={dark ? "Tema claro" : "Tema oscuro"}>{dark ? <Sun size={16}/> : <Moon size={16}/>}</button><button className="quick-action" onClick={() => setView("invoices")}><ReceiptText size={16}/> Cargar factura</button></div></header><main className="content" key={view}><div className="page-heading"><div><div className="eyebrow">SAN MIGUEL / CENTRO DE OPERACIONES</div><h1>{nav.find(n => n.id === view)?.label}</h1><p>{view === 'dashboard' ? 'Los números de tus locales y lo que necesita atención.' : 'Balbín y Perón · Mr Tasty'}</p></div><div className="filters">{view !== 'orders' && <Select value={location} onValueChange={setLocation}><SelectTrigger aria-label="Local"><Store size={16}/><SelectValue /></SelectTrigger><SelectContent>{Object.entries(names).map(([key, name]) => <SelectItem value={key} key={key}>{name}</SelectItem>)}</SelectContent></Select>}{(view === 'dashboard' || view === 'results') && <Select value={period} onValueChange={setPeriod}><SelectTrigger aria-label="Mes"><SelectValue /></SelectTrigger><SelectContent>{(finances.length ? Array.from(new Set(finances.map(f => f.period))).reverse() : [period]).map(p => <SelectItem value={p} key={p}>{periodLabel(p)}</SelectItem>)}</SelectContent></Select>}</div></div>
 {needsLogin && <div className="notice"><div>Para guardar registros, <a href="/login">iniciá sesión</a>.</div></div>} {view === 'dashboard' ? <><div className="period-note"><span className="status-dot"/> {!finances.length ? 'Todavía no hay estados de resultados cargados. Los números aparecen cuando se conecta Google Sheets desde Conexiones.' : period === latestComplete ? `${periodLabel(period)} es el último mes comparable con ventas y gastos cargados en ambos locales.` : 'Los importes corresponden a las planillas importadas.'}{!!finances.length && <button onClick={() => showResults('history')}>Ver detalle <ArrowUpRight size={15}/></button>}</div><div className="metrics"><Metric title="Facturación" value={compact(revenue)} subtitle="Ventas declaradas · ARS" delta={delta}/><Metric title="Gastos registrados" value={compact(expenses)} subtitle="Según categorías del EERR"/><Metric title="Resultado del mes" value={compact(profit)} subtitle={complete ? 'Utilidad declarada en las planillas' : 'Faltan datos o validaciones'} accent/><Metric title="Margen sobre ventas" value={pct(profit !== null && revenue ? profit / revenue : null)} subtitle="Resultado / facturación"/></div><div className="main-grid"><section className="panel chart-panel"><div className="panel-heading"><div><h2>Cómo viene la facturación</h2><p>Comparación mensual por local · pesos argentinos</p></div><span className="subtle">Tocá un mes para ver el detalle</span></div><div className="chart chart-clickable"><ResponsiveContainer width="100%" height="100%" minWidth={0} initialDimension={{ width: 700, height: 280 }}><BarChart data={chart} barGap={7} margin={{ top: 15, right: 16, left: 0, bottom: 0 }}><CartesianGrid vertical={false} stroke={chartColors.grid}/><XAxis dataKey="period" axisLine={false} tickLine={false} tick={{ fill: chartColors.axis, fontSize: 13 }}/><YAxis tickFormatter={v => `${v / 1e6} M`} axisLine={false} tickLine={false} width={55} tick={{ fill: chartColors.axis, fontSize: 12 }}/><Tooltip formatter={v => money(Number(v))} cursor={{ fill: chartColors.cursor }}/><Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 13, paddingTop: 10 }}/>{(location === 'all' || location === 'balbin') && <Bar isAnimationActive={!reducedMotion} name="Balbín" dataKey="balbin" fill={chartColors.balbin} radius={[5, 5, 0, 0]} maxBarSize={32} onClick={(_, i) => { const k = chart[i]?.key; if (k) setOpenPeriod(current => current === k ? null : k); }}>{chart.map(c => <Cell key={c.key} fillOpacity={openPeriod && openPeriod !== c.key ? 0.32 : 1}/>)}</Bar>} {(location === 'all' || location === 'peron') && <Bar isAnimationActive={!reducedMotion} name="Perón" dataKey="peron" fill={chartColors.peron} radius={[5, 5, 0, 0]} maxBarSize={32} onClick={(_, i) => { const k = chart[i]?.key; if (k) setOpenPeriod(current => current === k ? null : k); }}>{chart.map(c => <Cell key={c.key} fillOpacity={openPeriod && openPeriod !== c.key ? 0.32 : 1}/>)}</Bar>}</BarChart></ResponsiveContainer></div><div className={"period-detail-wrap" + (openPeriod ? " open" : "")}><div className="period-detail-inner">{openPeriod && <PeriodDetail period={openPeriod} finances={finances} location={location} onClose={() => setOpenPeriod(null)} onOpenResults={() => { setPeriod(openPeriod); showResults("history"); }}/>}</div></div></section><section className="panel attention"><div className="panel-heading"><h2>Para revisar</h2><span className="count-badge">3</span></div><Action index="01" title={incompletePeriod ? `Revisar cierre de ${periodLabel(incompletePeriod)}` : "Revisar el último cierre"} text="Comprobá importes faltantes y alertas de conciliación." onClick={() => { if (incompletePeriod) setPeriod(incompletePeriod); showResults('history'); }}/><Action index="02" title="Confirmar stock disponible" text="El stock histórico tiene referencias a Munro." onClick={() => setView('orders')}/><Action index="03" title={connected ? "Revisar las conexiones" : "Activar las conexiones"} text="Sheets, facturas y la bandeja de Gmail." onClick={() => setView('connections')}/></section></div><section className="panel"><div className="panel-heading"><div><h2>Local por local</h2><p>La misma lectura para los dos equipos.</p></div><button className="text-button" onClick={() => showResults('history')}>Abrir resultados <ArrowUpRight size={16}/></button></div><Table><TableHeader><TableRow><TableHead>Local</TableHead><TableHead>Facturación</TableHead><TableHead>Gastos</TableHead><TableHead>Resultado</TableHead><TableHead>Margen</TableHead><TableHead>Estado</TableHead></TableRow></TableHeader><TableBody>{records.map(f => <TableRow key={f.id}><TableCell><div className="location-cell"><span className={'local-icon ' + f.location}><Store size={19}/></span><div><strong>{names[f.location]}</strong><small>{f.location === 'balbin' ? 'San Miguel 1' : 'San Miguel 2'}</small></div></div></TableCell><TableCell>{money(f.revenue)}</TableCell><TableCell>{money(f.expenses)}</TableCell><TableCell className="positive">{f.complete ? money(f.profit) : 'Pendiente'}</TableCell><TableCell>{f.complete ? pct(f.margin) : 'Pendiente'}</TableCell><TableCell><span className={'badge ' + (f.complete ? 'good' : 'warning')}>{f.complete ? 'Cargado' : 'Por revisar'}</span></TableCell></TableRow>)}</TableBody></Table>{!records.length && <p className="chart-note">Sin datos para este período. Los estados de resultados se importan desde las planillas una vez activada la conexión con Google.</p>}</section><OperatingInsights location={location} period={period} counts={counts} finances={finances}/><LedgerOverview location={location} onOpen={() => showResults('ledger')}/><div className="footnote">Fuente: EERR importados y actualizaciones de Sheets cuando están activas. “Cargado” indica presencia de datos, no un cierre contable aprobado. Los meses incompletos conservan sus alertas.</div></> : view === 'orders' ? <Orders location={location} counts={counts}/> : view === 'invoices' ? <Invoices location={location} onChanged={() => { }}/> : view === 'results' ? <Results location={location} period={period} finances={finances} initialTab={resultMode}/> : view === 'people' ? <Candidates location={location}/> : <Connections onCounts={setCounts} onFinances={setFinances} onConnected={() => setConnected(true)}/>} 
