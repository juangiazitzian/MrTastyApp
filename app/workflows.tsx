...
import { averageDailyUse, categories, ConfiguredSupplier, consumptionSegments, Count, forecast, isConfigured, Kind, nextOrderLabel, Supplier, weekdaysLabel, safeCsv, schemas, signedAmount, skus, Sku, StoredRecord, suggestOrder, suppliers, todayAR } from '@/lib/domain';
...
function SupplierHead({ supplier, children }: {
    supplier: Supplier;
    children?: ReactNode;
}) {
...
/**
 * Proveedor con calendario pero sin catálogo.
 *
 * Se lista igual para que el aviso de «hoy toca pedir» funcione desde el primer
 * día. Sin insumos, bulto ni plazo de entrega no se sugiere ninguna cantidad, y
 * la tarjeta lo dice en vez de aparentar que está configurado.
 */
...
function SupplierSection({ supplier, chosen, stock, orders }: {
    ...
    return <section className="panel supplier-card"><SupplierHead supplier={supplier}><button className="primary" onClick={openCount}><Plus size={15}/> Cargar conteo</button><button className="secondary" onClick={() => openOrder(true)}><FileText size={15}/> Armar pedido</button></SupplierHead><Table><TableHeader><TableRow><TableHead>Insumo</TableHead><TableHead>Tenés</TableHead><TableHead>Usás por semana</TableHead><TableHead>Pedir</TableHead></TableRow></TableHeader><TableBody>{rows.map(r => <TableRow key={r.item.id}><TableCell><strong>{r.item.label}</strong><small className="cell-meta">bulto de {units(r.item.pack, r.item.unit)}</small></TableCell><TableCell>{r.last ? <>{units(r.last.quantity, r.item.unit)}<small className="cell-meta">{r.stale ? '⚠ conteo vencido · ' : ''}{new Date(r.last.at).toLocaleDateString('es-AR')}</small></> : <span className="muted">Sin conteo</span>}</TableCell><TableCell>{r.avg.perDay === null ? <span className="muted">Faltan conteos</span> : <>{units(Math.round(r.avg.perDay * 7), r.item.unit)}<small className="cell-meta">{r.avg.weeks} {r.avg.weeks === 1 ? 'semana medida' : 'semanas medidas'}{r.avg.preliminary ? ' · preliminar' : ''}{r.avg.discarded ? ` · ${r.avg.discarded} descartada` : ''}</small></>}</TableCell><TableCell>{r.qty === null ? <span className="muted">—</span> : <><strong className={r.qty > 0 ? 'order-qty' : ''}>{r.qty > 0 ? `${r.qty / r.item.pack} ${r.qty / r.item.pack === 1 ? 'bulto' : 'bultos'}` : 'Nada'}</strong><small className="cell-meta">{r.qty > 0 ? units(r.qty, r.item.unit) : 'el stock alcanza'}{r.incoming ? ` · ${units(r.incoming, r.item.unit)} en camino` : ''}</small></>}</TableCell></TableRow>)}</TableBody></Table>{rows.every(r => r.qty === null) && <p className="chart-note">Para sugerir cantidades hacen falta dos conteos del mismo insumo: sin eso no hay consumo medible y no se arriesga un número. Cargá el conteo de esta semana y la próxima ya vas a ver cantidades.</p>}<div className="supplier-foot"><button className="text-button" onClick={() => openOrder(false)}>Cargar un pedido pasado</button>{rows.some(r => r.qty !== null && r.qty > 0) && <button className="secondary" onClick={exportCsv}><Download size={15}/> Exportar CSV</button>}</div>{!!history.length && <details><summary>Pedidos anteriores ({history.length})</summary><Table><TableHeader><TableRow><TableHead>Fecha</TableHead><TableHead>Entrega</TableHead><TableHead>Estado</TableHead><TableHead>Detalle</TableHead></TableRow></TableHeader><TableBody>{history.map(o => <TableRow key={o.id}><TableCell>{String(o.payload.createdFor)}</TableCell><TableCell>{deliveryDate(o.payload, supplier.leadDays)}{o.payload.deliveredAt ? '' : ' (estimada)'}</TableCell><TableCell><span className={'badge ' + (o.payload.status === 'received' ? 'good' : 'warning')}>{o.payload.status === 'received' ? 'Recibido' : 'Borrador'}</span></TableCell><TableCell>{((o.payload.lines ?? []) as { sku: string; quantity: number }[]).map(l => `${supplier.items.find(i => i.id === l.sku)?.label ?? l.sku}: ${l.quantity}`).join(' · ')}</TableCell></TableRow>)}</TableBody></Table></details>}<Dialog open={!!sheet} onOpenChange={o => !o && setSheet(null)}><DialogContent className="tasty-dialog"><DialogHeader><DialogTitle>{sheet === 'count' ? 'Cargar conteo' : draft.status === 'received' ? 'Cargar pedido pasado' : 'Armar pedido'}</DialogTitle><DialogDescription>{sheet === 'count' ? `Los seis insumos de ${supplier.label} en ${names[chosen]}. Dejá en blanco lo que no se contó.` : `Cantidades en bultos. ${names[chosen]}.`}</DialogDescription></DialogHeader>{sheet === 'count' ? <form className="form-grid" onSubmit={saveCount}><Field label="Fecha y hora del conteo"><input required type="datetime-local" value={draft.countedAt || ''} onChange={e => field('countedAt', e.target.value)}/></Field>{supplier.items.map(i => <Field key={i.id} label={`${i.label} (${i.unit})`}><input type="number" min="0" step="1" placeholder="sin contar" value={draft[i.id] ?? ''} onChange={e => field(i.id, e.target.value)}/></Field>)}<button className="primary full" disabled={saving}>{saving ? 'Guardando…' : 'Guardar conteo'}</button></form> : <form className="form-grid" onSubmit={saveOrder}><Field label="Fecha del pedido"><input required type="date" value={draft.createdFor || ''} onChange={e => field('createdFor', e.target.value)}/></Field><Field label="Fecha de entrega (opcional)"><input type="date" value={draft.deliveredAt || ''} onChange={e => field('deliveredAt', e.target.value)}/></Field>{supplier.items.map(i => <Field key={i.id} label={`${i.label} · bulto de ${units(i.pack, i.unit)}`}><input type="number" min="0" step="1" placeholder="0" value={draft[i.id] ?? ''} onChange={e => field(i.id, e.target.value)}/></Field>)}<div className="full"><label className="check-line"><Checkbox checked={draft.status === 'received'} onCheckedChange={v => field('status', v ? 'received' : 'draft')}/> Ya entró la mercadería</label></div><button className="primary full" disabled={saving}>{saving ? 'Guardando…' : 'Guardar pedido'}</button></form>}</DialogContent></Dialog></section>;
}
